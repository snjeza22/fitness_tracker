...
async function getAllRoutines() {
  ...
  newRoutines.forEach((newRoutine) => {
    const matchingIndexes = routines.filter((r) => {
      return r.id === newRoutine.id;
    });

    newRoutine.activities = [];

    matchingIndexes.forEach((activity) => {
...
async function getAllPublicRoutines() {
  ...
  const publicRoutines = routinePbl.filter((routine) => {
    return routine.isPublic == true;
  });
...
async function getPublicRoutinesByUser({ id, username }) {
